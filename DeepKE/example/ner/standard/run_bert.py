...
class TrainNer(BertForTokenClassification):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            #label_list: ['O', 'B-CON', 'I-CON', 'B-ARI', 'I-ARI', 'B-EXT', 'I-EXT', '[CLS]', '[SEP]']
            
            class_weights = torch.tensor([1.0,  12.0631, 12.0631, 25055.1333, 25055.1333, 187913.5, 187913.5, 1.0, 1.0,1.0], dtype=torch.float).to(logits.device)
...
def main(cfg):
    ...
    if cfg.do_eval:
        ...
        eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=cfg.eval_batch_size * cfg.n_gpu)   
        model.eval()
        ...
        # 处理None值，这里将None替换为'O'标签
        y_true = [['O' if label is None else label for label in labels] for labels in y_true]
        y_pred = [['O' if label is None else label for label in labels] for labels in y_pred]
