...
def SetDatabase(conf: dict, config: ConfigManager):
    ...
    if conf.get('key') == 'api':
        # 更新为正确的配置键名
        config.set('api.api_key', conf.get('value').get('api_key'))
        config.set('api.model_name', conf.get('value').get('model_name'))
        config.set('api.base_url', conf.get('value').get('base_url'))
